...
An idle actor accepts one invocation. A reentrant invocation permits another invocation to enter; an undecorated invocation, including a socket hook by default, blocks all new invocations until it completes and its state is committed. Already-running reentrant continuations may still resume while that undecorated invocation awaits. Multiple reentrant invocations may overlap. Direct `this.method()` calls remain part of the current invocation and inherit its mode; the decorator does not create another invocation for a nested call. Actor-to-actor remote calls remain unsupported.
