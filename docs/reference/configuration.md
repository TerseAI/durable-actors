...
Hosted actor analytics use PostgreSQL through `DURABLE_ACTORS_POSTGRES_URL`, including on Cloud SQL. Startup applies the analytics schema migrations before accepting traffic. All control-plane instances sharing this database see the same request history, metrics, queue waits, and WebSocket history. Local `dev` continues to use `request-traces.sqlite3` in its data directory.
...
Retention cleanup runs once per minute in bounded batches. Analytics queries use project-scoped indexes and a ten-second statement timeout. Request streams also poll the shared database every five seconds to discover events written by other instances. Trace collection remains best effort before the database commit; host queues and report failures can still lose events. Existing in-memory production history cannot be recovered by this migration. PostgreSQL's retention is independent of the local SQLite limit of 10,000 events.
