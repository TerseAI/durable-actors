...
| ---------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `DURABLE_ACTORS_PROJECT_ID`        | `local` for loopback connections; required remotely             | Actor project ID; use the same value in the server registration and backend.                      |
...
| `DURABLE_ACTORS_SECRET`            | Optional locally; required remotely | Server API key. Keep it on the backend, without surrounding whitespace.                           |

Local CLI commands and backend clients need no connection settings with the defaults. Loopback origins (`localhost`, `127.0.0.1`, and `[::1]`) default to project `local` and omit authentication when no secret is configured. Set `DURABLE_ACTORS_SECRET` on both `dev` and your backend to enable local authentication. If you override the project or port, use matching settings in your backend. Remote connections require an explicit project ID and secret.
