...
Durable Actors runs stateful actors in regional sandbox hosts while a control plane owns placement, leases, and durable state coordination.
...
**Provider boot gap**:
The Modal-owned interval between scheduled sandbox creation and the actor-host process beginning. Durable Actors records its surrounding boundaries and the Modal resource identifiers, while Modal's supported dashboard timeline remains the source for scheduled, started, and ready transitions.
