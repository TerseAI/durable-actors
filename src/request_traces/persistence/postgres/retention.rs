...
use super::{PostgresTracePersistence, write_transaction};
...
impl PostgresTracePersistence {
    ...
    pub(crate) async fn prune_batch(&self) -> Result<u64> {
        ...
        let rows = transaction
            .query(
                "SELECT project_id, position FROM durable_actors_traces
                 WHERE received_at < now() - $1::double precision * interval '1 second'
        ...
        for (project, positions) in projects {
            transaction
                .query_one(
                    "SELECT project_id FROM durable_actors_trace_projects
                     WHERE project_id = $1
                     FOR UPDATE",
                    &[&project],
                )
            ...
            let row = transaction
                .query_one(
                    "WITH deleted AS (
                         DELETE FROM durable_actors_traces
                         WHERE project_id = $1 AND position = ANY($2)
                         RETURNING position
                     )
                     SELECT COUNT(*), MAX(position) FROM deleted",
                    &[&project, &positions],
                )
                .await?;
            ...
            transaction
                .execute(
                    "UPDATE durable_actors_trace_projects
                     SET pruned = GREATEST(pruned, $2), evicted = evicted + $3
                     WHERE project_id = $1",
