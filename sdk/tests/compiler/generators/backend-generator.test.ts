...
test("generates backend grants without a frontend socket client", async () => {
    const code = (await generateTypeScript({ version: 1, actors: [] })).get("index.d.ts")!
...
test("groups public state and method arguments and results under each actor", async t => {
    ...
    const files = await generateTypeScript(new ActorCompiler().compileContract(entrypoint))
    assert.ok(files.has("runtime/index.d.ts"))
    await writeArtifacts(root, files)
    await writeFile(
        path.join(root, "consumer.ts"),
        `
...
test("generates callable typed backend stubs in a consumer without actor source or private dependencies", async t => {
    ...
    const bundled = await build({
        entryPoints: [path.join(consumer, "index.js")],
        outfile,
        bundle: true,
        format: "esm",
        platform: "node",
...
async function writeArtifacts(root: string, files: ReadonlyMap<string, string>) {
...
async function checkUsageExamples(root: string, files: ReadonlyMap<string, string>) {
    const code = files.get("index.d.ts")!
    const examples = [...code.matchAll(/@example\n((?:[ \t]*\* .*\n)+)/g)].map(match =>
        match[1]!.replace(/^[ \t]*\* /gm, "").trim()
    )
    const [imports, ...snippets] = examples
    await writeArtifacts(path.join(root, "generated"), files)
