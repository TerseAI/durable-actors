...
`da observe` never requires a secret when connecting to `da dev`, even when application routes use one. The dev runtime binds only to localhost; hosted observability still uses the server's configured authentication.
