...
test("formats generated sections and provides concise usage examples", async t => {
    ...
    assert.match(
        code,
        /Use this file to call actors from your backend and connect to them from your frontend\.\n \* @example\n \* import \{ actors, clients, ActorProxy, type ActorAuthorization \} from "\.\/generated\/index\.js"/
    )
    assert.match(code, /namespace clients \{\n    export namespace Room \{/)
    assert.match(code, /        history: string\[\]/)
    assert.match(
        code,
        /Connect to an actor from your frontend[\s\S]*?@example\n \* const client = clients.Room.get\("actor-id"\)\n \* await client.connect\(\)\n \*\/\nexport const clients/
    )
    assert.match(
        code,
...
test("groups public state and method arguments and results under each actor", async t => {
    ...
    await writeFile(
        path.join(root, "consumer.ts"),
        `
        import { actors, clients } from "./index.js"
        ...
        const socketState: clients.ChatRoom.State = state
        const args: Args = [2]
...
test("generates callable typed backend stubs in a consumer without actor source or private dependencies", async t => {
    ...
    assert.equal(browserModule.clients.ChatRoom.get("room-123").status, "idle")
...
async function checkUsageExamples(root: string, code: string) {
    ...
    await writeFile(
        consumer,
        `${imports}\ndeclare const metadata: clients.Room.Metadata\n${snippets.map(example => `{\n${example}\n}`).join("\n")}`
    )
    checkTypes(consumer)
}
