...
function proxySource(contracts: readonly SocketContract[]): string {
    const actors = contracts
        .map(
            ({ actorName }) =>
                `[${JSON.stringify(actorName)}]: {} as import("little-actors/proxy").ProxyActor<actors.${actorName}.Metadata>`
        )
        .join(", ")
...
${contracts.map(({ actorName }) => `[${JSON.stringify(actorName)}]: import("little-actors/proxy").ProxyActor<actors.${actorName}.Metadata>`).join("\n")}
}> {
