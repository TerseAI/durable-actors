...
import { createActorStub as $createActorStub, SocketProxy as $SocketProxy } from "durable-actors/generated"
...
function proxySource(contracts: readonly SocketContract[]): string {
    const actors = contracts
        .map(
            ({ actorName }) =>
                `[${JSON.stringify(actorName)}]: {} as import("durable-actors/proxy").ProxyActor<actors.${actorName}.Metadata>`
        )
        .join(", ")
...
${contracts.map(({ actorName }) => `[${JSON.stringify(actorName)}]: import("durable-actors/proxy").ProxyActor<actors.${actorName}.Metadata>`).join("\n")}
}> {
    constructor(options: import("durable-actors/proxy").SocketProxyOptions = {}, dependencies: import("durable-actors/proxy").SocketProxyDependencies = {}) {
        super({ ${actors} }, options, dependencies)
    }

    handle(authorization: ActorAuthorization): Promise<import("durable-actors/proxy").SocketGrant> {
        return super.handle(authorization)
    }

    static handle(authorization: ActorAuthorization, options: import("durable-actors/proxy").SocketProxyOptions = {}): Promise<import("durable-actors/proxy").SocketGrant> {
