...
async function main(): Promise<void> {
    const [directory, entrypoint, output] = process.argv.slice(2)
    if (!directory || !entrypoint || !output)
        throw new Error("Expected project directory, actor entrypoint, and output directory")
    process.chdir(directory)
    const contract = parsePublicContract(new ActorCompiler().compileContract(entrypoint))
    ...
    await buildActor(entrypoint, artifact)
