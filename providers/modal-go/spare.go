...
func (p *provider) buildCode(ctx context.Context, request buildCodeRequest) (map[string]any, error) {
	...
	sb, err := p.api.Create(ctx, app, image, &modal.SandboxCreateParams{Command: []string{"sleep", "120"}, Timeout: 2 * time.Minute, Regions: []string{region}, Cloud: modalCloud(request.CanonicalRegion), CPU: 1, CPULimit: 1, MemoryMiB: 4096, MemoryLimitMiB: 4096})
	if err != nil {
		return nil, err
	}
	defer sb.Detach()
