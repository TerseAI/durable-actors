...
async fn history_counts_pruned_events_without_counting_duplicate_appends() -> Result<()> {
    ...
    store
        ...
        .await?;
    let page = store.history(&HistoryQuery::default()).await?;
...
async fn history_is_bounded_and_rejects_live_or_incompatible_cursors() -> Result<()> {
    ...
    store
        ...
        .await?;
    let first = store.history(&HistoryQuery::default()).await?;
    ...
    for query in [
        ...
        assert!(store.history(&query).await.is_err());
    }
    Ok(())
}
