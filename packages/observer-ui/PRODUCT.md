...
Overview class search and residency filters lead into the existing Actors inspector. Live actor classes and instances sort first. Selecting an instance opens its live and saved requests, filtered by both class and ID, alongside current WebSocket connections. Instance rows show up to three operation-name bubbles and an overflow count; instance details show the full ordered waiting line. The host reports operations admitted to the actor mailbox but not yet started, excluding activation and earlier socket/network buffers. Queue inventory streams through the existing host reports, coalesced over 100 ms. Missing reporting is unavailable, not zero. Clickable rows use native buttons without chevrons; request rows have consistent heights and full details in a Radix-backed shadcn Sheet. WebSockets lists current connections and metadata. The prior Overview implementation was UI-only work against existing APIs, with no backend or SDK contract changes and no durable trace archive in its scope. Saved request history now extends the Requests workflow; deployment metadata, identity, fabricated trends, and outgoing-write statistics remain outside scope. [BACKEND-GAPS.md](BACKEND-GAPS.md) records the absent contracts for a separate backend task.
