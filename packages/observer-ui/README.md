...
The standalone console includes a Requests view. Actor classes and instances with live residency appear first. Instance rows include compact operation-name bubbles for waiting requests; clicking an instance shows the full waiting line, requests, and WebSocket connections; both live and saved requests are scoped to its class and ID. Request rows stay at a consistent height, with full values in a Radix-backed shadcn Sheet opened by Details. To embed it, render `<RequestObserver client={client} />` using the same styles and `HttpObserverClient`. Pass `actor={{ actorType: "Room", actorId: "lobby" }}` to scope the view to one instance. Its optional `watchRequests(onPage, signal, after?)` client method consumes `<prefix>/requests/events`; proxy that stream to the admin-only `/v1/observe/requests/events` control-plane endpoint using server-side credentials. The local CLI provides this proxy automatically.

SSE `requests` events contain `{ epoch, cursor, capacity, evicted, dropped, persistenceFailed, records, nextCursor, resumeCursor, reset }`. The first event replays the available history; later events carry new records. Sequence numbers identify records within a storage generation; a new epoch or `reset` replaces the live window. `resumeCursor` is an opaque token passed back to `watchRequests` on reconnect. Each record includes a stable `eventId`, `requestId`, `hostId`, `sessionId`, `actorType`, `actorId`, `kind` (`method` or `websocket`), `operation`, nullable `connectionId`, `startedAtMs`, `durationMs`, nullable `queueWaitMs`, and `outcome`. Outcomes are `completed`, `failed`, `rejected`, `rerouted`, or `interrupted`.

Total includes queue wait, actor processing, and actor-state persistence; queue wait includes the WebSocket message queue. Timings exclude the caller’s network round trip. Rows arrive after an attempt finishes and its trace commits. The live UI retains 500 records per control plane. The local runtime saves up to 10,000 events in `request-traces.sqlite3`; IDs and replay cursors survive restart. The optional `query({ sql, params }, signal)` client method enables History. The observer owns its SQL filters and pagination in `src/request-sql.ts`; the server returns `{ rows, truncated }`. The HTTP adapter posts SQL to `/api/observe/query`, and the CLI proxy forwards it to `/v1/observe/query` with server-side credentials. The public SQLite views are `request_events` (event fields and original JSON) and `request_history` (generation, ingestion watermark, retention boundary, total). SQL is read-only and restricted to observability data, with bounded execution and results; see the [SQL contract and limits](../../docs/reference/cli.md#request-timings). Live-only custom clients can omit `query`.

A `persistenceFailed` warning means some events could not be saved. Hosted servers currently use in-memory SQLite without cross-replica sharing; a durable hosted adapter can implement append, SQL query, and live replay. Reuse requires a compatible public SQL schema/dialect and server-enforced project isolation. Collection is best effort and known delivery losses are shown; process crashes can also lose unreported records. See [request timing semantics](../../docs/reference/cli.md#request-timings).

Actor inventory optionally includes `waiting: [{ id, operation }]` on each instance. Entries are ordered by actor-mailbox admission and removed when execution starts. They exclude activation and upstream socket/network buffers. Null or omitted means unavailable; `[]` means no waiting work. The existing inventory stream carries updates, coalesced over 100 ms by the host. Expired or superseded host sessions cannot report a waiting line for their former instances. This is current queue state; retained request traces still provide historical queue-wait timings.
